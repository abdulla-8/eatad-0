...
<li>
    <a href="{{ route('insurance.users.index', $company->company_slug) }}"
        ...
        {{ t($company->translation_group . '.users_management') }}
                        ...
                        <li>
                            <a href="{{ route('insurance.settings.index', ['companyRoute' => $company->company_slug]) }}"
                                class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('insurance.settings.*') ? 'text-dark-900' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}"
                                style="{{ request()->routeIs('insurance.settings.*') ? 'background: ' . $company->primary_color : '' }}">
                                <svg class="w-5 h-5 {{ $isRtl ? 'ml-3' : 'mr-3' }}" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
