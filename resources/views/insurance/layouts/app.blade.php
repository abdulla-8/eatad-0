...
<head>
    ...
    <title>@yield('title', t('insurance.dashboard')) - {{ t('insurance.dashboard') }}</title>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3 {{ $isRtl ? 'space-x-reverse' : '' }}">
                    ...
                    <div class="flex items-center space-x-2 {{ $isRtl ? 'space-x-reverse' : '' }}">
                        ...
                        <h1 class="font-bold text-lg">{{ t('insurance.dashboard') }}</h1>
                    </div>
                </div>
        ...
        <div class="flex lg:flex-row">
                ...
                <div class="p-6 border-b border-gray-700">
                    <div class="flex items-center space-x-3 {{ $isRtl ? 'space-x-reverse' : '' }}">
                        <div class="w-12 h-12 bg-white rounded-xl p-2 shadow-lg">
                            <svg class="w-full h-full text-green-600" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z">
                                </path>
                        ...
                        <div>
                            <h1 class="text-lg font-bold">{{ t('insurance.dashboard') }}</h1>
                            <p class="text-gold-400 text-sm">Insurance Company</p>
                        </div>
                    </div>
                </div>
                <!-- Navigation -->
                    ...
                    <ul class="space-y-2">
                        <li>
                            <a href="{{ route('insurance.dashboard') }}"
                                class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('insurance.dashboard') ? 'bg-gold-500 text-dark-900' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3 {{ $isRtl ? 'space-x-reverse' : '' }}">
                            <div class="w-10 h-10 bg-gold-500 rounded-lg flex items-center justify-center">
                                <span
                                    class="text-dark-900 font-bold">{{ substr(auth('insurance_company')->user()->legal_name, 0, 1) }}</span>
                            </div>
                            <div class="hidden lg:block">
                                <p class="text-white text-sm font-medium">
                                    {{ auth('insurance_company')->user()->legal_name }}</p>
                                <p class="text-gray-400 text-xs">Insurance Company</p>
                            </div>
                        </div>
                        <form method="POST" action="{{ route('insurance.logout') }}">
                            @csrf
                            <button type="submit"
            ...
            <main class="flex-1 min-h-screen">
                ...
                <header class="hidden lg:block bg-white border-b border-gray-200 px-8 py-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900">@yield('title', t('insurance.dashboard'))</h2>
                            <p class="text-gray-600">{{ t('auth.welcome_back') }},
                                {{ auth('insurance_company')->user()->legal_name }}</p>
                        </div>
                        <x-language-switcher
