...
<head>
    ...
    <title>{{ t('auth.register') }} - {{ t('insurance.dashboard') }}</title>
    ...
    <div class="min-h-screen flex items-center justify-center p-6">
        <div class="w-full max-w-2xl">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-gray-400 text-sm">{{ t('insurance.dashboard') }}</p>
            </div>
            ...
            <div class="bg-dark-800/80 backdrop-blur-xl rounded-2xl p-6 border border-gold-500/20 shadow-2xl">
                ...
                <form method="POST" action="{{ route('insurance.register') }}" class="space-y-4">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" 
                                   ...
                                   placeholder="{{ t('auth.company_legal_name_placeholder', 'Insurance Company Legal Name') }}"
                                   required>
                        </div>
                        ...
                        <div>
                            <label for="employee_count" class="block text-white text-sm font-medium mb-1">
                                {{ t('auth.employee_count', 'Employee Count') }} <span class="text-gray-400 text-xs">({{ t('auth.optional', 'Optional') }})</span>
                            </label>
                            <input type="number" 
                                   id="employee_count" 
                                   name="employee_count" 
                    ...
                    <div>
                        <label for="office_address" class="block text-white text-sm font-medium mb-1">
                            {{ t('auth.office_address', 'Office Address') }} <span class="text-gray-400 text-xs">({{ t('auth.optional', 'Optional') }})</span>
                        </label>
                        <textarea id="office_address" 
                                  name="office_address" 
                                  rows="2"
                                  class="w-full px-3 py-2 bg-dark-900/50 border border-gold-500/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gold-500/50 focus:border-gold-500/50 transition-all text-sm"
                                  placeholder="{{ t('auth.office_address_placeholder', 'Full office address') }}">{{ old('office_address') }}</textarea>
                    </div>
                    ...
                    <div class="text-center">
                        <p class="text-gray-400 text-sm">
                            ...
                            <a href="{{ route('insurance.login') }}" class="text-gold-400 hover:text-gold-300 font-medium">
                                {{ t('auth.login') }}
                            </a>
                        </p>
                    </div>
