{{-- resources/views/insurance/users/create.blade.php --}}
@extends('insurance.layouts.app')

@section('title', t($company->translation_group . '.add_new_user'))
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <nav class="flex items-center space-x-2 rtl:space-x-reverse text-sm text-gray-500 mb-4">
                ...
                <span class="text-gray-900 font-medium">{{ t($company->translation_group . '.add_new_user') }}</span>
            </nav>
            ...
            <div class="flex items-center gap-4 rtl:gap-4">
                ...
                <div>
                    <h1 class="text-4xl font-bold text-gray-900 mb-2">{{ t($company->translation_group . '.add_new_user') }}</h1>
                    <p class="text-lg text-gray-600">{{ t($company->translation_group . '.add_new_user_for_company') }} <span class="font-semibold" style="color: {{ $company->primary_color }};">{{ $company->legal_name }}</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl border-0 overflow-hidden">
            ...
            <div class="px-8 py-6 border-b border-gray-100" style="background: linear-gradient(135deg, {{ $company->primary_color }}15 0%, {{ $company->primary_color }}05 100%);">
                <div class="flex items-center gap-4 rtl:gap-4">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">{{ t($company->translation_group . '.user_information') }}</h2>
                        <p class="text-sm text-gray-600">{{ t($company->translation_group . '.fill_user_details') }}</p>
                    </div>
                </div>
            </div>

            <!-- Form Body -->
            <form method="POST" action="{{ route('insurance.users.store', $company->company_slug) }}" class="p-8">
                @csrf
                ...
                <div class="mb-8">
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div class="group">
                            ...
                            <div class="relative">
                                <input type="text" name="national_id" value="{{ old('national_id') }}" required maxlength="14"
                ...
                <div class="mb-8">
                    <div class="flex items-center gap-3 rtl:gap-3 mb-6">
                        ...
                        <h3 class="text-lg font-semibold text-gray-900">{{ t($company->translation_group . '.security_information') }}</h3>
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <div class="group">
                            <label class="block text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2 rtl:gap-2">
                                ...
                                {{ t($company->translation_group . '.password') }} 
                                <span class="text-red-500">*</span>
                            </label>
                            <div class="relative">
                                <input type="password" name="password" required
                                    ...
                                    style="focus:border-color: {{ $company->primary_color }};"
                                    placeholder="{{ t($company->translation_group . '.enter_password') }}">
                                <div class="absolute inset-y-0 ltr:right-3 rtl:left-3 flex items-center pointer-events-none">
                                    <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                                    </svg>
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                <input type="password" name="password_confirmation" required
                                    ...
                                    style="focus:border-color: {{ $company->primary_color }};"
                                    placeholder="{{ t($company->translation_group . '.confirm_password_placeholder') }}">
                                <div class="absolute inset-y-0 ltr:right-3 rtl:left-3 flex items-center pointer-events-none">
                                    <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>
                                    </svg>
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-8 border-t border-gray-100">
                    <button type="submit" 
                        ...
                        <span class="relative z-10 flex items-center justify-center gap-3 rtl:gap-3">
                            ...
                            {{ t($company->translation_group . '.create_user') }}
                        </span>
