...
<div class="relative" style="z-index: 9999;">
    <button type="button" 
        ...
        <span class="text-xs font-medium">{{ $currentLanguage->name ?? 'Language' }}</span>
        <svg class="w-4 h-4 {{ $isRtl ? 'mr-2' : 'ml-2' }} transition-transform duration-200" 
    ...
    <div id="languageDropdown{{ $uniqueId }}" 
         class="absolute {{ $isRtl ? 'left-0' : 'right-0' }} mt-2 w-44 bg-white rounded-lg shadow-lg border border-gray-200 py-1 opacity-0 invisible transform scale-95 transition-all duration-200 origin-top-{{ $isRtl ? 'left' : 'right' }}"
         style="z-index: 10000;">
        @foreach($activeLanguages as $language)
            <a href="{{ route('language.change', $language->code) }}" 
               class="flex items-center px-4 py-2 text-xs hover:bg-gray-50 transition-colors duration-150 {{ app()->getLocale() == $language->code ? 'bg-gold-50 text-gold-800' : 'text-gray-700' }}">
                <div class="w-5 h-5 rounded bg-gold-500 flex items-center justify-center {{ $isRtl ? 'ml-2' : 'mr-2' }} text-white text-xs font-bold">
                    {{ strtoupper($language->code) }}
                </div>
                <div class="flex-1">
                    <div class="font-medium">{{ $language->name }}</div>
                    <div class="text-xs text-gray-500 flex items-center">
                        <span class="w-2 h-2 rounded-full {{ $language->direction == 'rtl' ? 'bg-orange-400' : 'bg-blue-400' }} {{ $isRtl ? 'ml-1' : 'mr-1' }}"></span>
                        {{ strtoupper($language->direction) }}
                        @if($language->is_default)
                            <span class="text-gold-600 {{ $isRtl ? 'mr-2' : 'ml-2' }}">• {{ __('admin.default') }}</span>
                        @endif
                    </div>
                </div>
