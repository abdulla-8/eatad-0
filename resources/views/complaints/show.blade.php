...
@section('title', t('complaints' . '.complaint_details'))
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        @endif
        <div class="flex-1">
            <h1 class="text-3xl font-bold text-gray-900">{{ t('complaints' . '.complaint_details') }}</h1>
          
            
            <!-- عرض اسم الشركة لمستخدمي شركة التأمين -->
            @if($userType === 'insurance_user' && $user->company)
                <div class="mt-2 flex items-center gap-2">
                    <span class="text-sm font-medium text-gray-700">{{ t('complaints' . '.company') }}:</span>
        ...
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-sm font-medium text-gray-600">{{ t('complaints' . '.complaint_id') }}:</span>
            ...
            @if($userType === 'insurance_user')
                <div class="mb-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-3 flex items-center gap-2">
                        ...
                        {{ t('complaints' . '.complainant_information') }}
                    </h3>
                    <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-blue-700 text-sm">{{ t('complaints' . '.name') }}:</span>
                                <span class="text-blue-900 font-semibold">{{ $user->full_name }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-blue-700 text-sm">{{ t('complaints' . '.policy_number') }}:</span>
                                <span class="text-blue-900 font-mono">{{ $user->policy_number }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-blue-700 text-sm">{{ t('complaints' . '.phone') }}:</span>
                                <span class="text-blue-900" dir="ltr">{{ $user->phone }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="font-medium text-blue-700 text-sm">{{ t('complaints' . '.email') }}:</span>
                                <span class="text-blue-900" dir="ltr">{{ $user->email }}</span>
                            </div>
                            @if($user->company)
                                <div class="flex items-center gap-2 md:col-span-2">
                                    <span class="font-medium text-blue-700 text-sm">{{ t('complaints' . '.company') }}:</span>
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-600">{{ t('complaints' . '.click_to_view_attachment') }}</p>
                        </div>
            ...
            <div class="border-t pt-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="font-medium text-gray-900">{{ t('complaints' . '.complaint_submitted') }}</span>
                                <span class="text-sm text-gray-500">{{ $complaint->created_at->diffForHumans() }}</span>
                            </div>
                            <p class="text-sm text-gray-600">
                                {{ t('complaints' . '.submitted_by_you') }}
                                @if($userType === 'insurance_user' && $user->company)
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="font-medium text-gray-900">{{ t('complaints' . '.current_status') }}</span>
                            ...
                            <p class="text-sm text-gray-600">
                                @if($complaint->is_read)
                                    {{ t('complaints' . '.complaint_has_been_reviewed') }}
                                @else
                                    {{ t('complaints' . '.complaint_awaiting_review') }}
                                @endif
                            </p>
                            @if($complaint->is_read)
            ...
            <div class="border-t pt-6 mt-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">{{ t('complaints' . '.additional_information') }}</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-gray-50 rounded-lg p-4">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="font-medium text-gray-700">{{ t('complaints' . '.submission_date') }}</span>
                        </div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="font-medium text-gray-700">{{ t('complaints' . '.last_update') }}</span>
                        </div>
                    ...
                    @if($userType === 'insurance_user')
                        <div class="bg-blue-50 rounded-lg p-4">
                            <div class="flex items-center gap-2 mb-2">
                                ...
                                <span class="font-medium text-blue-700">{{ t('complaints' . '.complainant_type') }}</span>
                            </div>
                            <p class="text-sm text-blue-600 font-medium">{{ t('complaints' . '.insurance_user') }}</p>
                        </div>
                        ...
                        @if($user->company)
                            <div class="bg-blue-50 rounded-lg p-4">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <span class="font-medium text-blue-700">{{ t('complaints' . '.insurance_company') }}</span>
                                </div>
