...
<head>
    ...
    <title>@yield('title', t('dealer.dashboard')) - {{ t('dealer.dashboard') }}</title>
...
<body class="bg-gray-100 {{ app()->getLocale() == 'ar' ? 'font-cairo' : 'font-inter' }}">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3 {{ $isRtl ? 'space-x-reverse' : '' }}">
                    ...
                    <div class="flex items-center space-x-2 {{ $isRtl ? 'space-x-reverse' : '' }}">
                        ...
                        <h1 class="font-bold text-lg">{{ t('dealer.dashboard') }}</h1>
                    </div>
                </div>
        ...
        <div class="flex lg:flex-row">
            ...
            <aside id="sidebar" class="fixed lg:static inset-y-0 {{ $isRtl ? 'right-0' : 'left-0' }} z-40 w-72 bg-dark-900 text-white transform {{ $isRtl ? 'translate-x-full' : '-translate-x-full' }} lg:translate-x-0 transition-transform duration-300 ease-in-out lg:flex lg:flex-col">
                ...
                <div class="p-6 border-b border-gray-700">
                    <div class="flex items-center space-x-3 {{ $isRtl ? 'space-x-reverse' : '' }}">
                        ...
                        <div>
                            <h1 class="text-lg font-bold">{{ t('dealer.dashboard') }}</h1>
                            <p class="text-gold-400 text-sm">Parts Dealer</p>
                        </div>
                    </div>
                </div>
                ...
                <nav class="mt-6 px-4">
                    <ul class="space-y-2">
                        <li>
                            <a href="{{ route('dealer.dashboard') }}" class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('dealer.dashboard') ? 'bg-gold-500 text-dark-900' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
                        ...
                        <li>
                            <a href="#" class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200 text-gray-300 hover:bg-gray-800 hover:text-white opacity-60 cursor-not-allowed">
                                ...
                                {{ t('dealer.my_profile') }}
                                <span class="text-xs bg-gray-700 px-2 py-1 rounded {{ $isRtl ? 'mr-auto' : 'ml-auto' }}">Soon</span>
                            </a>
                        </li>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3 {{ $isRtl ? 'space-x-reverse' : '' }}">
                            <div class="w-10 h-10 bg-gold-500 rounded-lg flex items-center justify-center">
                                <span class="text-dark-900 font-bold">{{ substr(auth('parts_dealer')->user()->legal_name, 0, 1) }}</span>
                            </div>
                            <div class="hidden lg:block">
                                <p class="text-white text-sm font-medium">{{ auth('parts_dealer')->user()->legal_name }}</p>
                                <p class="text-gray-400 text-xs">Parts Dealer</p>
                            </div>
                        </div>
            ...
            <main class="flex-1 min-h-screen">
                ...
                <header class="hidden lg:block bg-white border-b border-gray-200 px-8 py-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900">@yield('title', t('dealer.dashboard'))</h2>
                            <p class="text-gray-600">{{ t('auth.welcome_back') }}, {{ auth('parts_dealer')->user()->legal_name }}</p>
                        </div>
                        <x-language-switcher class="bg-gray-50 border border-gray-200 text-gray-700 hover:bg-gray-100" />
                    </div>
                </header>
    ...
    <script>
        // Mobile menu toggle (نفس الكود من الأدمن)
