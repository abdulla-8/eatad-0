...
@section('title', t('admin.language_management'))
...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
    <div>
        <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 mb-2">{{ t('admin.language_management') }}</h1>
        <nav class="flex text-sm">
            <a href="{{ route('admin.dashboard') }}" class="text-gray-500 hover:text-gold-600">{{ t('admin.dashboard') }}</a>
            <span class="mx-2 text-gray-400">></span>
            <span class="text-gold-600 font-medium">{{ t('admin.languages') }}</span>
        </nav>
    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="hidden md:block">
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-{{ $isRtl ? 'right' : 'left' }} text-xs font-semibold text-gray-600 uppercase">{{ t('admin.language_name') }}</th>
                        <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase">{{ t('admin.language_code') }}</th>
                        <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase">{{ t('admin.direction') }}</th>
                        <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase">{{ t('admin.status') }}</th>
                        <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase">{{ t('admin.default') }}</th>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        @if($language->code == app()->getLocale())
                                            <span class="bg-gold-100 text-gold-800 text-xs px-2 py-1 rounded-full">{{ t('admin.current') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                                ...
                                <div class="flex items-center justify-center space-x-2 {{ $isRtl ? 'space-x-reverse' : '' }}">
                                    ...
                                    @if(!$language->is_default && $language->is_active)
                                        <form method="POST" action="{{ route('admin.languages.default', $language->id) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="p-2 rounded-lg bg-gold-100 text-gold-600 hover:bg-gold-200 transition-colors" onclick="return confirm('{{ t('admin.confirm_default') }}')">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-2 {{ $isRtl ? 'space-x-reverse' : '' }} text-xs">
                                    ...
                                    @if($language->code == app()->getLocale())
                                        <span class="bg-gold-100 text-gold-800 px-2 py-1 rounded-full">{{ t('admin.current') }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-center space-x-3 {{ $isRtl ? 'space-x-reverse' : '' }} pt-3 border-t border-gray-200">
                        <form method="POST" action="{{ route('admin.languages.toggle', $language->id) }}" class="inline">
                            ...
                            <button type="submit" class="flex items-center px-3 py-2 rounded-lg text-sm font-medium {{ $language->is_active ? 'bg-orange-100 text-orange-700 hover:bg-orange-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }} transition-colors">
                                ...
                                {{ $language->is_active ? t('admin.deactivate') : t('admin.activate') }}
                            </button>
                        </form>
                        ...
                        @if(!$language->is_default && $language->is_active)
                            <form method="POST" action="{{ route('admin.languages.default', $language->id) }}" class="inline">
                                @csrf
                                <button type="submit" class="flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-gold-100 text-gold-700 hover:bg-gold-200 transition-colors" onclick="return confirm('{{ t('admin.confirm_default') }}')">
                                    <svg class="w-4 h-4 {{ $isRtl ? 'ml-1' : 'mr-1' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>
                                    </svg>
                                    {{ t('admin.set_default') }}
                        ...
                        @if($language->is_active && $language->code != app()->getLocale())
                            <a href="{{ route('language.change', $language->code) }}" class="flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors">
                                ...
                                {{ t('admin.switch_to') }} {{ $language->name }}
                            </a>
                        @endif
                    </div>
                </div>
    ...
    @else
        <div class="p-12 text-center">
            ...
            <h3 class="text-xl font-semibold text-gray-900 mb-2">{{ t('admin.no_languages') }}</h3>
            <p class="text-gray-600">{{ t('admin.no_languages_desc') }}</p>
        </div>
    @endif
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
    <div class="bg-blue-50 rounded-xl p-6 border border-blue-200">
        <div class="flex items-center mb-3">
            ...
            <h3 class="font-bold text-gray-900">{{ t('admin.language_info') }}</h3>
        </div>
        <p class="text-gray-700 text-sm">{{ t('admin.language_info_desc') }}</p>
    </div>
    ...
    <div class="bg-gold-50 rounded-xl p-6 border border-gold-200">
        <div class="flex items-center mb-3">
            ...
            <h3 class="font-bold text-gray-900">{{ t('admin.default_language') }}</h3>
        </div>
        <p class="text-gray-700 text-sm">{{ t('admin.default_language_desc') }}</p>
    </div>
    ...
    <div class="bg-green-50 rounded-xl p-6 border border-green-200">
        <div class="flex items-center mb-3">
            ...
            <h3 class="font-bold text-gray-900">{{ t('admin.switch_language') }}</h3>
        </div>
        <p class="text-gray-700 text-sm">{{ t('admin.switch_language_desc') }}</p>
    </div>
</div>
