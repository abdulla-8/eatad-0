    ...
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between">
        <div class="flex items-center space-x-4 {{ $isRtl ? 'space-x-reverse' : '' }} mb-4 lg:mb-0">
            <div class="w-16 h-16 bg-gold-500 rounded-xl p-3">
                <img src="{{ asset('logo.png') }}" alt="{{ __('admin.site_name') }}" class="w-full h-full object-contain">
            </div>
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold">{{ __('admin.welcome_message') }}</h1>
                <p class="text-gray-300">{{ __('admin.dashboard_description') }}</p>
            </div>
        </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-600">{{ __('admin.total_languages') }}</p>
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-600">{{ __('admin.current_language') }}</p>
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-600">{{ __('admin.text_direction') }}</p>
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-600">{{ __('admin.system_status') }}</p>
                <p class="text-xl font-bold text-green-600">{{ __('admin.active') }}</p>
            </div>
            <div class="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center">
                <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-6 space-y-4">
            <a href="{{ route('admin.languages.index') }}" 
               class="flex items-center p-4 bg-gray-50 hover:bg-gold-50 rounded-lg transition-colors duration-200 group border border-gray-200 hover:border-gold-300">
                ...
                <span class="font-semibold text-gray-900 group-hover:text-gold-800 transition-colors duration-200">{{ __('admin.manage_languages') }}</span>
            </a>
            ...
            <div class="flex items-center p-4 bg-gray-50 rounded-lg border border-gray-200 opacity-60">
                ...
                <span class="font-semibold text-gray-600">{{ __('admin.manage_users') }} ({{ __('admin.coming_soon') }})</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div class="p-6 space-y-4">
            <div class="flex items-center p-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <div class="flex-1">
                    <p class="font-medium text-gray-900">{{ __('admin.admin_logged_in') }}</p>
                    <p class="text-sm text-gray-600">{{ now()->format('Y-m-d H:i') }}</p>
                </div>
                <span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full font-medium">{{ __('admin.login') }}</span>
            </div>
            ...
            <div class="flex items-center p-3 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div class="flex-1">
                    <p class="font-medium text-gray-900">{{ __('admin.languages_loaded') }}</p>
                    <p class="text-sm text-gray-600">{{ __('admin.system_initialized') }}</p>
                </div>
                <span class="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full font-medium">{{ __('admin.system') }}</span>
            </div>
        </div>
    </div>
</div>
