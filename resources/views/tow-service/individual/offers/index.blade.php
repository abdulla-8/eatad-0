...
@section('title', t('tow.individual_offers'))
...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-600 mt-1">{{ t('tow.manage_individual_offers') }}</p>
        </div>
    ...
    @else
        ...
        <div class="bg-white rounded-xl shadow-sm border">
            <div class="p-12 text-center">
                ...
                <p class="text-gray-600">{{ t('tow.no_individual_offers_description') }}</p>
            </div>
        </div>
    @endif
</div>
...
document.getElementById('acceptForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch(`{{ route('tow-service.individual.offers.accept', '__ID__') }}`.replace('__ID__', currentOfferId), {
            method: 'POST',
            headers: {
...
document.getElementById('rejectForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch(`{{ route('tow-service.individual.offers.reject', '__ID__') }}`.replace('__ID__', currentOfferId), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
