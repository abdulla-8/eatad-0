...
@section('title', t('tow.company_offers'))
...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ t('tow.tow_offers') }}</h1>
            <p class="text-gray-600 mt-1">{{ t('tow.manage_tow_offers') }}</p>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="bg-white rounded-lg border px-4 py-3 text-center shadow-sm">
                ...
                <div class="text-xs text-gray-600">{{ t('tow.total_offers') }}</div>
            </div>
            <div class="bg-white rounded-lg border px-4 py-3 text-center shadow-sm">
                <div class="text-2xl font-bold text-yellow-600">{{ $stats['pending'] }}</div>
                <div class="text-xs text-gray-600">{{ t('tow.pending') }}</div>
            </div>
            <div class="bg-white rounded-lg border px-4 py-3 text-center shadow-sm">
                <div class="text-2xl font-bold text-green-600">{{ $stats['accepted'] }}</div>
                <div class="text-xs text-gray-600">{{ t('tow.accepted') }}</div>
            </div>
            <div class="bg-white rounded-lg border px-4 py-3 text-center shadow-sm">
                <div class="text-2xl font-bold text-red-600">{{ $stats['rejected'] }}</div>
                <div class="text-xs text-gray-600">{{ t('tow.rejected') }}</div>
            </div>
        </div>
    </div>
<!-- Filters -->
    ...
    <div class="p-6">
        <form method="GET" class="flex flex-col lg:flex-row gap-6 items-end">
            ...
            <div class="flex gap-3">
                <button type="submit" 
                        class="px-6 py-2.5 bg-gold-600 text-dark-900 rounded-lg font-medium hover:bg-gold-500 transition-colors whitespace-nowrap">
                    {{ t('tow.filter') }}
                </button>
    ...
    @else
        ...
        <div class="bg-white rounded-xl shadow-sm border">
            <div class="p-12 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 mb-2">{{ t('tow.no_tow_offers') }}</h3>
                <p class="text-gray-600">{{ t('tow.no_tow_offers_description') }}</p>
            </div>
        </div>
    @endif
</div>
...
<div id="acceptModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl max-w-md w-full">
        <div class="p-6 border-b">
            <h3 class="text-xl font-bold">{{ t('tow.accept_tow_offer') }}</h3>
        </div>
        
        <form id="acceptForm" class="p-6 space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('tow.estimated_pickup_time') }}</label>
                <input type="datetime-local" name="estimated_pickup_time" 
                       class="w-full border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent px-4 py-2.5"
                       min="{{ now()->addMinutes(30)->format('Y-m-d\TH:i') }}">
            </div>
            
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('tow.notes') }}</label>
                <textarea name="notes" rows="3" 
                          class="w-full border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent px-4 py-2.5"
                          placeholder="{{ t('tow.additional_notes') }}"></textarea>
            </div>
...
<div id="rejectModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl max-w-md w-full">
        <div class="p-6 border-b">
            <h3 class="text-xl font-bold">{{ t('tow.reject_tow_offer') }}</h3>
        </div>
        
        <form id="rejectForm" class="p-6 space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('tow.rejection_reason') }}</label>
                <textarea name="rejection_reason" rows="4" 
                          class="w-full border-gray-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent px-4 py-2.5"
                          placeholder="{{ t('tow.why_rejecting') }}"></textarea>
            </div>
    ...
    <div class="bg-white rounded-xl max-w-lg w-full">
        <div class="p-6 border-b">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-xl font-bold text-green-800">{{ t('tow.offer_accepted_successfully') }}</h3>
            </div>
        </div>
        
        <div class="p-6 space-y-6">
            <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                <p class="text-green-800 text-sm">{{ t('tow.tow_request_assigned_to_you') }}</p>
            </div>
            
            <div>
                <h4 class="font-bold text-gray-900 mb-3">📱 {{ t('tow.driver_tracking_link') }}</h4>
                <p class="text-gray-600 text-sm mb-3">{{ t('tow.share_link_with_driver') }}</p>
                ...
                <div class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded">
                    <p class="text-blue-800 text-xs">
                        <strong>{{ t('tow.instructions') }}:</strong> {{ t('tow.driver_link_instructions') }}
                    </p>
                </div>
            </div>
...
<div id="driverLinkModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl max-w-md w-full">
        <div class="p-6 border-b">
            <h3 class="text-xl font-bold">📱 {{ t('tow.driver_tracking_link') }}</h3>
        </div>
        
        <div class="p-6 space-y-4">
            <p class="text-gray-600 text-sm">{{ t('tow.share_link_with_driver') }}</p>
...
function copyDriverLink() {
    ...
    showMessage('{{ t("tow.link_copied") }}', 'success');
}
...
document.getElementById('acceptForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch(`{{ route('tow-service.company.offers.accept', '__ID__') }}`.replace('__ID__', currentOfferId), {
            method: 'POST',
            headers: {
...
document.getElementById('rejectForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch(`{{ route('tow-service.company.offers.reject', '__ID__') }}`.replace('__ID__', currentOfferId), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
