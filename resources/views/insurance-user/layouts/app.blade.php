...
<head>
    ...
    <link
        href="{{ $fontLink ?? 'https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;500;600;700&display=swap' }}"
        rel="stylesheet">
                ...
                @if (function_exists('get_active_languages'))
                    <div class="relative">
                        <select onchange="changeLanguage(this.value)"
                            class="bg-gray-50 border border-gray-200 text-gray-700 hover:bg-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50">
        ...
        <div class="flex lg:flex-row">
            ...
            <aside id="sidebar"
                ...
                <nav class="mt-6 px-4">
                    <ul class="space-y-2">
                        <li>
                            <a href="{{ route('insurance.user.dashboard', ['companySlug' => $company->company_slug]) }}"
                                class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('insurance.user.dashboard') ? 'text-dark-900' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}"
                                style="{{ request()->routeIs('insurance.user.dashboard') ? 'background: ' . $company->primary_color : '' }}">
                        ...
                        <li>
                            <a href="{{ route('insurance.user.claims.index', ['companySlug' => $company->company_slug]) }}"
                                class="flex items-center px-4 py-3 rounded-lg transition-colors duration-200 {{ request()->routeIs('insurance.user.claims.*') ? 'text-dark-900' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}"
                                style="{{ request()->routeIs('insurance.user.claims.*') ? 'background: ' . $company->primary_color : '' }}">
                                <svg class="w-5 h-5 {{ $isRtl ? 'ml-3' : 'mr-3' }}" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <li>
                            <a href="#"
                                ...
                                <svg class="w-5 h-5 {{ $isRtl ? 'ml-3' : 'mr-3' }}" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M3 8l7.89 1.26a2 2 0 001.11 0L20 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z">
                                    </path>
                                </svg>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3 {{ $isRtl ? 'space-x-reverse' : '' }}">
                            <div class="w-10 h-10 rounded-lg flex items-center justify-center"
                                style="background: {{ $company->primary_color }};">
                                <span
                                    class="text-white font-bold text-sm">{{ substr(auth('insurance_user')->user()->full_name, 0, 2) }}</span>
                            </div>
                            <div class="hidden lg:block">
                                <p class="text-white text-sm font-medium">
                                    {{ Str::limit(auth('insurance_user')->user()->full_name, 20) }}</p>
                                <p class="text-gray-400 text-xs">
                                    {{ t($company->translation_group . '.insured_member', 'Insured Member') }}</p>
                            </div>
                        </div>
                        <form method="POST"
                            action="{{ route('insurance.user.logout', ['companySlug' => $company->company_slug]) }}">
                            @csrf
                            <button type="submit"
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-gray-600">
                                ...
                                {{ auth('insurance_user')->user()->full_name }}</p>
                        </div>
                        @if (function_exists('get_active_languages'))
                            <div class="relative">
                                <select onchange="changeLanguage(this.value)"
